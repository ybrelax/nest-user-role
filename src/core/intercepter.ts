import { WebSiteDataModel, PageModelItem } from './types';
...
// 匹配路径
export const matchPath = (website: WebSiteDataModel | undefined, targetPath: string) => {
  if (!website) return;
  ...
  Object.keys(website || {}).forEach((path) => {
    ...
    // 非必须容错：仅允许最后一个字符出现 *
    if (pathArr.slice(0, -1).find((i) => i === '*')) throw new Error('[matchPath] path include *');
